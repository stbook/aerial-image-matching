...
# instance image
im_ins = cv2.imread('000055.jpg', 1)
# reference image
im_ref = cv2.imread('000140.jpg', 1)

# load keypoints and descriptors from mat files for instance and reference image
ins_mat = io.loadmat('000055_sift.mat')
ref_mat = io.loadmat('000140_sift.mat')
...
np.save('matches_055.npy', np_matches)
#'''

np_matches = np.load('matches_055.npy')
...
plt.imshow(show_ins)
plt.savefig(base_parts_results_folder + 'show_ins_test.jpg', dpi=400)
plt.close()
...
plt.imshow(show_ref)
plt.savefig(base_parts_results_folder + 'show_ref_test.jpg', dpi=400)
plt.close()
...
plt.imshow(show_ins)
plt.savefig(base_parts_results_folder + 'show_ins_good_test.jpg', dpi=400)
plt.close()
...
plt.imshow(show_ref)
plt.savefig(base_parts_results_folder + 'show_ref_good_test.jpg', dpi=400)
plt.close()
#'''
...
plt.imshow(im_show)
plt.savefig(base_parts_results_folder + 'matches_test.jpg', dpi=(400))
plt.close()
#'''
...
opts_tolerance3 = 8
opts_minInliers = 6
opts_numRefinementIterations = 8

numMatches = len(good)
inliers = {}
H = {}

# keypoints satisfies second best ratio
x1 = ins_kp_good[0:2, :]
x2 = ref_kp_good[0:2, :]
...
plt.imshow(im_show)
plt.savefig(base_parts_results_folder + 'best_matches_test.jpg', dpi=(400))
plt.close()
#'''

'''
x1c = np.zeros((3, 7), np.float32)
x1c[0, :] = [-0.8130, -0.0355, 0.5072, -1.6358, 0.5717, 1.4410, -0.0355]
x1c[1, :] = [-1.2014, 0.7181, 0.3450, 1.0832, -0.1793, -1.4836, 0.7181]
x1c[2, :] = [1, 1, 1, 1, 1, 1, 1]

x2c = np.zeros((3, 7), np.float32)
x2c[0, :] = [-0.7187, -0.0870, 0.4350, -1.6522, 0.6532, 1.4569, -0.0870]
x2c[1, :] = [-1.1868, 0.7296, 0.3755, 1.0333, -0.1681, -1.5132, 0.7296]
x2c[2, :] = [1, 1, 1, 1, 1, 1, 1]
'''

S2 = np.zeros((3,3), np.float32)
S2[0, :] = [0.1490, 0, -20.8282]
S2[1, :] = [0, 0.0333, -13.0360]
S2[2, :] = [0, 0, 1]